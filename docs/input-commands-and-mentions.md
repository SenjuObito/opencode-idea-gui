...
### 2.1 触发与补全协调器
- **组件/Hook**：[`useChatInputCompletionsCoordinator.ts`](file:///i:/source/repos/opencode-idea-gui/webview/src/components/ChatInputBox/hooks/useChatInputCompletionsCoordinator.ts)
...
### 2.2 数据提供器架构 (`mentionProvider`)
- **文件位置**：[`mentionProvider.ts`](file:///i:/source/repos/opencode-idea-gui/webview/src/components/ChatInputBox/providers/mentionProvider.ts)
- **并发聚合**：
  ```typescript
...
- **实现位置**：[`FileHandler.ts`](file:///i:/source/repos/opencode-vscode-plugin/src/host/handlers/FileHandler.ts) 与 [`opencode-sdk-client.js`](file:///i:/source/repos/opencode-vscode-plugin/ai-bridge/services/opencode/opencode-sdk-client.js)
- **主通道（OpenCode SDK）**：
...
- **前端收敛**：
  - Webview 端 [`ContentBlockRenderer.tsx`](file:///i:/source/repos/opencode-idea-gui/webview/src/components/MessageItem/ContentBlockRenderer.tsx) 与 [`BashToolGroupBlock.tsx`](file:///i:/source/repos/opencode-idea-gui/webview/src/components/toolBlocks/BashToolGroupBlock.tsx) 自动将连续的 bash 命令聚合展示。
...
### 4.1 命令提供器与分组聚合
- **实现位置**：[`slashCommandProvider.ts`](file:///i:/source/repos/opencode-idea-gui/webview/src/components/ChatInputBox/providers/slashCommandProvider.ts)
- **命令来源聚合**：
...
### 4.2 本地拦截与后端分流执行
在 [`useMessageSender.ts`](file:///i:/source/repos/opencode-idea-gui/webview/src/hooks/useMessageSender.ts) 中，对输入的斜杠命令进行三级分流：
