...
- 宿主发送服务（`SessionSendService` / `OpenCodeSession`）推快照前先 `SessionState.trimMessagesFromRevertBoundary()`，按 `RevertState.messageID` 截掉边界之后。
...
- **宿主（IDEA Java）**：`OpenCodeMarkerParser`（解析 `[MESSAGE_REMOVED]`）、`OpenCodeMessageHandler.handleMessagesRemoved`、`SessionState.removeMessagesByIds` / `trimMessagesFromRevertBoundary`、`SessionSendService.updateSessionStateForSend`
...
- **自动化测试**：
  ...
  - `src/test/java/com/opencodebuddy/session/SessionStateTest.java`（`removeMessagesByIds` / `trimMessagesFromRevertBoundary` —— 宿主按 id 删除与边界裁剪）
