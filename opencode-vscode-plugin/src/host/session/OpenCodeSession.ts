...
import { isKnownSlashCommand } from './KnownCommands';
...
export class OpenCodeSession {
	...
	async sendShell(command: string, cwdOverride?: string | null): Promise<void> {
		...
		const normMode = normalizePermissionMode(this.state.getPermissionMode());
		const agent = normMode === 'plan' ? 'plan' : 'build';
		const params: Record<string, unknown> = {
			sessionId: this.state.getSessionId() ?? undefined,
			command: trimmed,
			model: this.state.getModel() ?? undefined,
...
/** 解析前导斜杠命令：'/name args...' → {command, arguments}；非命令或非预置命令返回 null。 */
function parseSlashCommand(text: string): { command: string; arguments: string } | null {
	...
	return { command: commandName, arguments: (m[2] ?? '').trim() };
}
