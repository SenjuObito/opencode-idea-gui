package com.opencodebuddy.action.dev;
...
public class OpenDevToolsAction extends AnAction {
    ...
    public OpenDevToolsAction() {
        super(
            OpenCodeBuddyBundle.message("action.openDevTools.text"),
            OpenCodeBuddyBundle.message("action.openDevTools.description"),
            null
        );
    }
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        List<DevToolsOption> options = List.of(
                new DevToolsOption(OpenCodeBuddyBundle.message("devtools.openEmbedded"), this::openEmbeddedDevTools),
                new DevToolsOption(OpenCodeBuddyBundle.message("devtools.openChrome"), this::openChromeDevTools),
                new DevToolsOption(OpenCodeBuddyBundle.message("devtools.copyDebugInfo"), this::copyDebugInfo)
        );

        ListPopup popup = JBPopupFactory.getInstance().createListPopup(
                new BaseListPopupStep<>(OpenCodeBuddyBundle.message("devtools.popupTitle"), options) {
                    @Override
    ...
    private void openEmbeddedDevTools(Project project) {
        ...
        if (browser == null) {
            ...
            showNotification(project, OpenCodeBuddyBundle.message("devtools.noBrowser"), NotificationType.WARNING);
            return;
        }
            ...
            if (browser.getCefBrowser() != null) {
                ApplicationManager.getApplication().invokeLater(() -> {
                    try {
                        ...
                        showNotification(project, OpenCodeBuddyBundle.message("devtools.openFailed", ex.getMessage()), NotificationType.ERROR);
                    }
                });
            }
        } catch (Exception ex) {
    ...
    private void openChromeDevTools(Project project) {
        ...
        if (port <= 0) {
            showNotification(project,
                    OpenCodeBuddyBundle.message("devtools.portNotAvailable"),
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            try {
                ...
                if (jsonResponse == null || jsonResponse.isEmpty()) {
                    ApplicationManager.getApplication().invokeLater(() ->
                            showNotification(project, OpenCodeBuddyBundle.message("devtools.noResponse", port), NotificationType.ERROR));
                    return;
                }
                ...
                if (devtoolsUrl != null) {
                    ...
                    ApplicationManager.getApplication().invokeLater(() -> {
                        ...
                        showNotification(project,
                                OpenCodeBuddyBundle.message("devtools.urlNotFound"),
                ...
                ApplicationManager.getApplication().invokeLater(() ->
                        showNotification(project, OpenCodeBuddyBundle.message("devtools.fetchFailed", ex.getMessage()), NotificationType.ERROR));
            }
        });
    }
    ...
    private void copyDebugInfo(Project project) {
        ...
        String debugInfo = OpenCodeBuddyBundle.message("devtools.debugInfoTitle") + "\n" +
                "━━━━━━━━━━━━━━━━━━━━━━\n" +
                "Port: " + port + "\n" +
                OpenCodeBuddyBundle.message("devtools.debugInfoTargetList", port) + "\n" +
                "━━━━━━━━━━━━━━━━━━━━━━\n\n" +
                OpenCodeBuddyBundle.message("devtools.debugInfoInstructions", port);

        copyToClipboard(debugInfo);
        showNotification(project, OpenCodeBuddyBundle.message("devtools.debugInfoCopied", port), NotificationType.INFORMATION);
    ...
    private void showNotification(Project project, String message, NotificationType type) {
        NotificationGroupManager.getInstance()
                .getNotificationGroup("OpenCode Buddy Notifications")
