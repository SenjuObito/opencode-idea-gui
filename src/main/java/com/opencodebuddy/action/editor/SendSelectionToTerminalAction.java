...
public class SendSelectionToTerminalAction extends AnAction implements DumbAware {
    ...
    SendSelectionToTerminalAction(@NotNull SelectionReferenceBuilder selectionReferenceBuilder) {
        super(
            OpenCodeBuddyBundle.message("action.sendToGui.text"),
            OpenCodeBuddyBundle.message("action.sendToGui.description"),
            null
        );
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        try {
            ...
            showError(project, OpenCodeBuddyBundle.message("send.failed", ex.getMessage()));
    ...
    private void sendToChatWindow(@NotNull Project project, @NotNull String text) {
        try {
            ...
            ToolWindow toolWindow = toolWindowManager.getToolWindow("OpenCode");
            ...
            if (toolWindow != null) {
                ...
                showError(project, OpenCodeBuddyBundle.message("send.toolWindowNotFound"));
            }

        } catch (Exception ex) {
            showError(project, OpenCodeBuddyBundle.message("send.sendToChatFailed", ex.getMessage()));
    ...
    private void showError(@Nullable Project project, @NotNull String message) {
        ...
        if (project != null) {
            ApplicationManager.getApplication().invokeLater(() -> {
                com.intellij.openapi.ui.Messages.showErrorDialog(project, message, OpenCodeBuddyBundle.message("dialog.error.title"));
            });
        }
    }
    ...
    private void showInfo(@Nullable Project project, @NotNull String message) {
        ...
        if (project != null) {
            ApplicationManager.getApplication().invokeLater(() -> {
                com.intellij.openapi.ui.Messages.showInfoMessage(project, message, OpenCodeBuddyBundle.message("dialog.info.title"));
            });
        }
    }
