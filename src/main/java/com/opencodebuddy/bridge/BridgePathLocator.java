...
import com.opencodebuddy.util.PluginMetadata;
...
final class BridgePathLocator {
    ...
    static void addPluginCandidates(List<File> possibleDirs) {
        ...
        try {
            ...
            if (!pluginsRoot.isEmpty()) {
                addPluginRootDirCandidates(possibleDirs, new File(pluginsRoot));
            }

            String systemPath = PathManager.getSystemPath();
            if (!systemPath.isEmpty()) {
                addPluginRootDirCandidates(possibleDirs, Paths.get(systemPath, "plugins").toFile());
            }
        } catch (Throwable t) {
    ...
    /**
     * Add {@code <pluginDir>/ai-bridge} for every installed plugin directory
     * under the given plugins root, plus the legacy/id-named dirs. The on-disk
     * plugin directory name follows rootProject.name and cannot be assumed.
     */
    private static void addPluginRootDirCandidates(List<File> possibleDirs, File pluginsRoot) {
        if (pluginsRoot == null || !pluginsRoot.isDirectory()) {
            return;
        }
        addCandidate(possibleDirs, new File(pluginsRoot, PLUGIN_DIR_NAME));
        addCandidate(possibleDirs, new File(pluginsRoot, PlatformUtils.getPluginId()));
        File[] installed = pluginsRoot.listFiles(File::isDirectory);
        if (installed != null) {
            for (File dir : installed) {
                addCandidate(possibleDirs, new File(dir, SDK_DIR_NAME));
            }
        }
    }
