...
public class McpServerHandler extends BaseMessageHandler {
    ...
    private void waitForBridgeAndFetchStatus(String cwd) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // Get server status (daemon opencode.getMcpStatus)
    ...
    private void waitForBridgeAndFetchTools(String serverId, String cwd, Gson gson) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // opencode manages MCP servers itself; per-server tool listing is
                // not exposed over the daemon. Reply with an empty tool list.
                ApplicationManager.getApplication().invokeLater(() -> {
                    JsonObject result = new JsonObject();
                    result.addProperty("serverId", serverId);
                    result.add("tools", new com.google.gson.JsonArray());
                    LOG.info("[McpServerHandler] MCP tool listing unsupported for opencode; returning empty list");
                    callJavaScript("window.updateMcpServerTools", escapeJs(gson.toJson(result)));
                });
            } catch (Exception e) {
    ...
    private void handleAddMcpServer(String content) {
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                String errorMsg = escapeJs(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("mcp.addServerFailedWithReason", e.getMessage()));
    ...
    private void handleUpdateMcpServer(String content) {
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                String errorMsg = escapeJs(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("mcp.updateServerFailedWithReason", e.getMessage()));
                callJavaScript("window.showError", errorMsg);
            });
        }
    }
    ...
    private void handleDeleteMcpServer(String content) {
        try {
            ...
            if (success) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    String reason = com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("mcp.serverNotFound");
                    String errorMsg = escapeJs(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message(
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                String errorMsg = escapeJs(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("mcp.deleteServerFailedWithReason", e.getMessage()));
                callJavaScript("window.showError", errorMsg);
            });
        }
    }
