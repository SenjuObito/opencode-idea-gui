    ...
    private void handleRewindFiles(String content) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // opencode rewinds files by reverting the session to the message
                // preceding the given user message.
                context.getOpenCodeSDKBridge().revert(sessionId, cwd, userMessageId)
                    .thenAccept(success -> {
                        LOG.info("[RewindHandler] Rewind result: success=" + success);
                        ...
                        if (success) {
                            ...
                            LOG.warn("[RewindHandler] Rewind failed");
                            callbackResult.addProperty("message", "Failed to restore files");
                        }
