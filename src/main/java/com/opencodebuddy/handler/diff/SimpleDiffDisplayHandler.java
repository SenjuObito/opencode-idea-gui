...
import com.opencodebuddy.util.ContentRebuildUtil;
import com.opencodebuddy.util.EditorFileUtils;
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = title != null ? title : OpenCodeBuddyBundle.message("diff.fileChange", fileName);
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
                            OpenCodeBuddyBundle.message("diff.editBefore", fileName),
                            OpenCodeBuddyBundle.message("diff.editAfter", fileName)
                    );
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = OpenCodeBuddyBundle.message("diff.tabName", fileName, edits.size());
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = title != null ? title : OpenCodeBuddyBundle.message("diff.editPreview", fileName);
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
                            OpenCodeBuddyBundle.message("diff.editPreviewCurrent", fileName),
                            OpenCodeBuddyBundle.message("diff.editPreviewAfter", fileName)
                    );
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    if (originalContent == null || originalContent.isEmpty()) {
                        diffTitle = (title != null ? title : OpenCodeBuddyBundle.message("diff.edit", fileName))
                                + " " + OpenCodeBuddyBundle.message("diff.editOnly");
                        leftLabel = "old_string";
                        rightLabel = "new_string";
                    } else {
                        diffTitle = title != null ? title : OpenCodeBuddyBundle.message("diff.edit", fileName);
                        leftLabel = OpenCodeBuddyBundle.message("diff.editBefore", fileName);
                        rightLabel = OpenCodeBuddyBundle.message("diff.editAfter", fileName);
                    }
