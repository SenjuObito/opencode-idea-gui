package com.opencodebuddy.handler.file;
...
public class FileExportHandler extends BaseMessageHandler {
    ...
    public boolean handle(String type, String content) {
        if ("save_markdown".equals(type)) {
            ...
            handleSaveFile(content, ".md", com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saveMarkdownDialog"));
            return true;
        } else if ("save_json".equals(type)) {
            LOG.info("[FileExportHandler] 处理: save_json");
            handleSaveFile(content, ".json", com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saveJsonDialog"));
            return true;
        }
        return false;
    }
    ...
    private void writeFileAsync(File fileToSave, String content) {
        CompletableFuture.runAsync(() -> {
            try (FileWriter writer = new FileWriter(fileToSave, StandardCharsets.UTF_8)) {
                ...
                notifySuccess(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saved"));
            } catch (IOException e) {
                LOG.error("[FileExportHandler] 保存文件失败: " + e.getMessage(), e);
                String errorDetail = e.getMessage() != null ? e.getMessage() : com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saveFailed");
                notifyError(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saveFailedWithReason", errorDetail));
            }
        });
    }
    ...
    private void notifyError(String message) {
        ApplicationManager.getApplication().invokeLater(() -> {
            String errorDetail = message != null ? message : com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.unknownError");
            String errorMsg = escapeJs(com.opencodebuddy.i18n.OpenCodeBuddyBundle.message("file.saveFailedWithReason", errorDetail));
            String jsCode = "if (window.addToast) { " +
