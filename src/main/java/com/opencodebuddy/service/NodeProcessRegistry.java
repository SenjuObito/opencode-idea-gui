 ...
 *       via {@link DaemonBridge#getDaemonProcessForInspection()}.</li>
...
public final class NodeProcessRegistry implements Disposable {
    ...
    public List<NodeProcessInfo> snapshot() {
        ...
        for (ClaudeChatWindow window : windows) {
            ...
            // -- DAEMON entry from the opencode daemon bridge --
            collectDaemonEntry(
                    result,
                    knownPids,
                    window.getDaemonBridge(),
                    tabProvider,
                    sessionId,
                    tabName,
                    now
        ...
        // When multiple IDEs (e.g. IDEA + PyCharm) both run OpenCode Buddy, each instance's
    ...
    static @Nullable String detectProviderFromCmd(String cmd) {
        ...
        if (lower.contains("daemon.js") || lower.contains("channel-manager")) {
            return "opencode";
        }
