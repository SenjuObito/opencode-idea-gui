...
import com.opencodebuddy.util.JBCefBrowserFactory;
import com.opencodebuddy.util.LanguageConfigService;
...
public class WebviewInitializer {
    ...
    public interface WebviewHost {
        Project getProject();
        com.opencodebuddy.provider.common.DaemonBridge getDaemonBridge();
        JPanel getMainPanel();
    ...
    public void createUIComponents() {
        ...
        com.opencodebuddy.bridge.BridgeDirectoryResolver sharedResolver = BridgePreloader.getSharedResolver();
        ...
        if (savedNodePath != null && !savedNodePath.trim().isEmpty()) {
            ...
            nodeDetector.setNodeExecutable(trimmed);
            NodeDetectionResult nodeResult = nodeDetector.verifyAndCacheNodePath(trimmed);
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            ...
            try {
                detected = nodeDetector.detectNodeWithDetails();
            } catch (Exception e) {
            ...
            if (detected != null && detected.isFound() && detected.getNodePath() != null) {
                ...
                nodeDetector.setNodeExecutable("node");
            }
    ...
    private void continueCreateUIComponentsAfterNodeSetup(NodeDetectionResult nodeResult) {
        ...
        // Prewarm the opencode daemon in background so the first user message
        // starts faster (serve + SSE subscription + session resolve).
        host.getDaemonBridge().ensureRunning();
        ...
        try {
            ...
            currentBridges.hidePanelQuery.addHandler((msg) -> {
                ...
                invokeLaterForToolWindow(() -> {
                    ...
                    ToolWindow toolWindow = ToolWindowManager.getInstance(project).getToolWindow("OpenCode");
    ...
    public void showErrorPanel() {
        String nodeExecutable = com.opencodebuddy.bridge.NodeDetector.getInstance().getNodeExecutable();
        String message = OpenCodeBuddyBundle.message(
            "error.nodeNotFound.message", nodeExecutable);

        JPanel errorPanel = ErrorPanelBuilder.build(
            OpenCodeBuddyBundle.message("error.nodeNotFound.title"),
            message,
            nodeExecutable,
    ...
    private void showVersionErrorPanel(String currentVersion) {
        ...
        String message = OpenCodeBuddyBundle.message(
            ...
            currentVersion, String.valueOf(minVersion),
            com.opencodebuddy.bridge.NodeDetector.getInstance().getNodeExecutable());
        ...
        switch (status) {
            case DISABLED_BY_REGISTRY:
                JPanel disabledPanel = ErrorPanelBuilder.buildCenteredPanel(
                        "⚠️",
                        OpenCodeBuddyBundle.message("toolwindow.jcefDisabled"),
                        OpenCodeBuddyBundle.message("toolwindow.jcefDisabledSolution"),
                        OpenCodeBuddyBundle.message("toolwindow.jcefEnableAction"),
                        this::enableJcefAndShowRestartPanel
                );
                replaceMainContent(disabledPanel);
                return;
            case OUTDATED_JBR:
                title = OpenCodeBuddyBundle.message("toolwindow.jcefOutdatedJbr");
                solution = OpenCodeBuddyBundle.message("toolwindow.jcefOutdatedJbrSolution");
                break;
            case ANDROID_STUDIO_PLUGIN_MISSING:
                title = OpenCodeBuddyBundle.message("toolwindow.jcefPluginMissing");
                solution = OpenCodeBuddyBundle.message("toolwindow.jcefPluginMissingSolution");
                break;
            default:
                title = OpenCodeBuddyBundle.message("toolwindow.jcefNotInstalled");
                solution = OpenCodeBuddyBundle.message("toolwindow.jcefNotInstalledSolution");
                break;
        }
    ...
    private void enableJcefAndShowRestartPanel() {
        ...
        JPanel panel = ErrorPanelBuilder.buildCenteredPanel(
                "✓",
                OpenCodeBuddyBundle.message("toolwindow.jcefRestartRequired"),
                OpenCodeBuddyBundle.message("toolwindow.jcefRestartRequiredSolution")
        );
        replaceMainContent(panel);
    }
    ...
    private void showJcefRemoteModeErrorPanel() {
        ...
        JPanel panel = ErrorPanelBuilder.buildCenteredPanel(
            "⚠️",
            OpenCodeBuddyBundle.message("toolwindow.jcefRemoteError"),
            OpenCodeBuddyBundle.message("toolwindow.jcefRemoteSolution"),
            OpenCodeBuddyBundle.message("toolwindow.jcefRemoteAction"),
    ...
    private void disableOutOfProcessJcefAndShowRestartPanel() {
        if (!JBCefBrowserFactory.disableOutOfProcessJcefInRegistry()) {
            ...
            JPanel failedPanel = ErrorPanelBuilder.buildCenteredPanel(
                    "⚠️",
                    OpenCodeBuddyBundle.message("toolwindow.jcefRemoteActionFailedTitle"),
                    OpenCodeBuddyBundle.message("toolwindow.jcefRemoteActionFailed")
            );
        ...
        JPanel panel = ErrorPanelBuilder.buildCenteredPanel(
                "✓",
                OpenCodeBuddyBundle.message("toolwindow.jcefOutOfProcessDisabled"),
                OpenCodeBuddyBundle.message("toolwindow.jcefOutOfProcessDisabledSolution")
        );
        replaceMainContent(panel);
    }
    ...
    private void showLoadingPanel() {
        JPanel panel = ErrorPanelBuilder.buildLoadingPanel(
            "⏳",
            OpenCodeBuddyBundle.message("toolwindow.extractingTitle"),
    ...
    /**
     * Node availability check: probe `node --version` and the bridge directory.
     */
    private boolean checkEnvironment() {
        try {
            ...
            if (!new java.io.File(bridgeDir, "daemon.js").exists()) {
                LOG.error("daemon.js not found at: " + bridgeDir.getAbsolutePath());
                return false;
            }
            return true;
        } catch (Exception e) {
