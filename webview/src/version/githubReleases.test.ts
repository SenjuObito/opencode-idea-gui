...
describe('githubReleases repository constants', () => {
  it('points to the correct opencode-idea-gui GitHub repository', () => {
    expect(GITHUB_REPO_OWNER).toBe('SenjuObito');
    expect(GITHUB_REPO_NAME).toBe('opencode-idea-gui');
    expect(GITHUB_REPO_URL).toBe('https://github.com/SenjuObito/opencode-idea-gui.git');
    expect(GITHUB_RELEASES_API).toBe(
      'https://api.github.com/repos/SenjuObito/opencode-idea-gui/releases'
    );
  });
});
...
describe('fetchGithubReleases logic & fallback', () => {
  ...
  it('returns parsed releases and writes to cache when online releases exist', async () => {
    ...
    expect(result.entries[0].version).toBe('1.0.0-opencode.1');
    expect(result.entries[0].date).toBe('2026-09-06');
    expect(result.entries[0].content.zh).toBe('### Features\n- OpenCode dedicated release');
    expect(result.entries[1].version).toBe('0.5.4');
